...
import trinetraLogo from "@/src/assets/trinetra-logo-transparent.png";
...
const CircularDotLoader = () => (
  <span className="relative block h-4 w-4">
    {Array.from({ length: 8 }).map((_, index) => (
      <span
        key={index}
        className="absolute left-1/2 top-1/2 h-1.5 w-1.5 rounded-full bg-current"
        style={{
          transform: `translate(-50%, -50%) rotate(${index * 45}deg) translateY(-7px)`,
          opacity: 0.2 + index * 0.1,
          animation: `dot-fade 0.9s linear ${index * 0.08}s infinite`,
        }}
      />
    ))}
  </span>
...
export const SignInPage = ({ className, onSuccess, onGoogleSignIn, googleButtonLabel }: SignInPageProps) => {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      localStorage.removeItem("trinetra_clerk_id");
      ...
      localStorage.setItem("trinetra_auth_mode", "otp");
      setMessage({
        type: "success",
  ...
  const handleGuestLogin = async () => {
    try {
      ...
      localStorage.setItem("trinetra_auth_mode", "guest");
  ...
  return (
    <div className={cn("relative flex min-h-screen w-full flex-col overflow-hidden bg-black text-on-surface", className)}>
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(0,0,0,0.04)_0%,rgba(0,0,0,0.84)_58%,rgba(0,0,0,1)_100%)]" />
      ...
      <div className="relative z-10 flex flex-1 flex-col">
        <div className="flex flex-1">
          <div className="flex flex-1 items-center justify-center px-4 py-8 sm:px-8 sm:py-10">
            <div className="w-full max-w-md rounded-[28px] border border-outline-variant/70 bg-black/72 p-7 shadow-[0_24px_80px_rgba(0,0,0,0.42)] backdrop-blur-md sm:p-8">
                ...
                {step === "email" ? (
                    ...
                    <div className="space-y-2">
                      <div className="mx-auto flex justify-center pb-1">
                        <div className="relative flex h-28 w-28 items-center justify-center sm:h-32 sm:w-32">
                          <div className="absolute inset-4 rounded-full bg-[radial-gradient(circle,rgba(152,207,227,0.12)_0%,rgba(152,207,227,0.04)_42%,transparent_74%)] blur-lg" />
                          <img
                            src={trinetraLogo}
                            alt="Trinetra logo"
                            className="relative h-full w-full object-contain opacity-95 brightness-110 contrast-110 [filter:drop-shadow(0_0_14px_rgba(152,207,227,0.12))]"
                          />
                        </div>
                      </div>
                      ...
                      {onGoogleSignIn ? (
                        <>
                          <button
                            ...
                            className="flex w-full items-center justify-center gap-2 rounded-full border border-outline-variant bg-black/70 px-4 py-3 text-on-surface transition-colors hover:border-primary/35 hover:bg-black/85"
                          >
                            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-sm text-primary">G</div>
                            <span>{googleButtonLabel || (mode === "signup" ? "Continue with Google" : "Sign in with Google")}</span>
                          </button>
                      ...
                      <form onSubmit={handleEmailSubmit}>
                        <div className="relative">
                          <input
                            ...
                            className="w-full rounded-full border border-outline-variant bg-black/55 px-4 py-3 text-center text-on-surface outline-none transition-colors placeholder:text-outline focus:border-primary/40"
                            required
                          />
                          <button
                            type="submit"
                          ...
                          >
                            {isSendingOtp ? <CircularDotLoader /> : <ArrowRight className="h-4 w-4" />}
                          </button>
                        </div>
                      </form>
                  ...
                  >
                    ...
                    <div className="w-full">
                      <div className="relative rounded-full border border-outline-variant bg-black/65 px-5 py-4">
                      ...
                      <motion.button
                        ...
                        className={`flex-1 rounded-full border py-3 font-medium transition-all duration-300 ${
                          code.every((d) => d !== "")
                            ...
                            : "cursor-not-allowed border-outline-variant bg-black/60 text-outline"
                        }`}
